...
function Kpi({ label, value, sub, color, testid }: { label: string; value: string; sub?: string; color?: string; testid?: string }) {
  return (
    <div style={TILE} data-testid={testid}>
      <div style={{ fontSize: 11, color: "#7a818a", marginBottom: 3 }}>{label}</div>
      <div style={{ fontSize: 22, fontWeight: 800, lineHeight: 1, color: color ?? "#1c2024" }}>{value}</div>
      {sub && <div style={{ fontSize: 11, color: "#9aa1a9", marginTop: 3 }}>{sub}</div>}
    </div>
  );
}

function LedgerRow({ label, amount, sign }: { label: string; amount: number; sign: "+" | "-" | "=" }) {
  const color = sign === "+" ? "var(--ot-gauge-good,#009e73)" : sign === "-" ? "var(--ot-gauge-bad,#d62828)" : "#1c2024";
  const prefix = sign === "+" ? "+" : sign === "-" ? "−" : "";
  return (
    <div style={{ display: "flex", justifyContent: "space-between", padding: "3px 0", borderTop: sign === "=" ? "1px solid #e2e5e9" : "none" }}>
      <span style={{ color: "#5a626b", fontWeight: sign === "=" ? 700 : 400 }}>{label}</span>
      <b style={{ color }}>{prefix}{fmtMoney(Math.abs(amount))}</b>
    </div>
  );
}
...
export function StatsDashboard({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  const satColor = netSat == null ? "#9aa1a9" : netSat >= 70 ? "var(--ot-gauge-good,#009e73)" : netSat >= 45 ? "#e69f00" : "var(--ot-gauge-bad,#d62828)";
    ...
    <>
      ...
      <div
        ...
        style={{
          ...
          maxHeight: "88vh",
          overflow: "auto",
          background: "var(--ot-color-surface,#f6f7f9)",
          borderRadius: 14,
      ...
      >
        <div style={{ display: "flex", alignItems: "center", marginBottom: 12 }}>
          ...
          <button
            ...
            onClick={onClose}
            style={{ marginLeft: "auto", border: "1px solid #d7dade", background: "#fff", borderRadius: 7, padding: "5px 12px", cursor: "pointer", font: "600 13px system-ui" }}
          >
            ✕ Close
          </button>
        </div>
        ...
        <div style={{ display: "flex", gap: 14, flexWrap: "wrap", alignItems: "flex-start" }}>
          ...
          <div style={{ flex: "1 1 240px", minWidth: 240 }}>
            ...
            <div style={{ background: "#fff", borderRadius: 10, padding: "10px 12px", border: "1px solid #eceef1" }}>
            ...
            <div style={{ background: "#fff", borderRadius: 10, padding: "8px 12px", border: "1px solid #eceef1" }}>
              {topPnl.length === 0 ? (
                <div style={{ color: "#9aa1a9", fontSize: 12 }}>No lines yet.</div>
              ) : (
                topPnl.map(({ l, p }) => (
                  <div key={l.lineId} style={{ display: "flex", alignItems: "center", gap: 6, padding: "2px 0" }}>
          ...
          <div style={{ flex: "1 1 240px", minWidth: 240 }}>
            ...
            <div style={{ background: "#fff", borderRadius: 10, padding: "10px 12px", border: "1px solid #eceef1" }}>
              {topRidership.length === 0 ? (
                <div style={{ color: "#9aa1a9", fontSize: 12 }}>No riders carried yet.</div>
              ) : (
        ...
        <div style={{ fontSize: 11, color: "#9aa1a9", marginTop: 8 }}>
